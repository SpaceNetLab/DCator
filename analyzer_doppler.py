...
def optimize_with_gurobi(sat_pos_cbf, initial_guess, sat_vel, doppler, freq, timelimit, ue_num):
    ...
    for i in range(len(sat_pos_cbf)): 
        dist_sqrt_var = model.addVar(lb=0, ub=GRB.INFINITY, name=f"dist_sqrt_{i}") 
        dist_sqrt_vars.append(dist_sqrt_var)

        doppler_var = model.addVar(lb=-GRB.INFINITY, ub=GRB.INFINITY, name=f"doppler_{i}")  
        doppler_vars.append(doppler_var)
        doppler_sq_var = model.addVar(lb=0, ub=GRB.INFINITY, name=f"doppler_sq_{i}")  
